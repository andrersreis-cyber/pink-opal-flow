-- ================================================
-- AUDITORIA FINAL - VERIFICAÇÃO COMPLETA DO SISTEMA
-- ================================================
-- Executar antes de merge para dev
-- Data: 24/11/2025

\echo '\n========================================='
\echo 'AUDITORIA FINAL - SISTEMA COMPLETO'
\echo '=========================================\n'

-- ================================================
-- 1. INTEGRIDADE DOS AGENDAMENTOS
-- ================================================
\echo '\n[1] VERIFICAÇÃO DE AGENDAMENTOS\n'

-- 1.1 Últimos 5 agendamentos criados
\echo '1.1 - Últimos 5 agendamentos (ordenados por data de criação):'
SELECT 
  a.id,
  a.cliente_id,
...
-- 1.2 Verificar agendamentos sem funcionário
\echo '\n1.2 - Agendamentos SEM funcionário (deveria estar vazio):'
SELECT 
...
-- 1.3 Verificar agendamentos com funcionário não habilitado
\echo '\n1.3 - Agendamentos com funcionário NÃO HABILITADO para o serviço:'
SELECT 
  a.id,
  c.nome as cliente,
  s.nome as servico,
  ...
  CASE 
    WHEN fs.funcionario_id IS NULL THEN '❌ NÃO HABILITADO'
    WHEN fs.ativo = false THEN '⚠️ VÍNCULO INATIVO'
    ELSE '✅ OK'
...
-- 1.4 Verificar consistência de duração
\echo '\n1.4 - Verificar se duração do agendamento corresponde ao serviço:'
SELECT 
  a.id,
  s.nome as servico,
  ...
  CASE 
    WHEN a.duracao_minutos = s.duracao_minutos THEN '✅ OK'
    ELSE '⚠️ DIVERGENTE'
  END as status
FROM agendamentos a
JOIN servicos s ON a.servico_id = s.id
...
-- ================================================
\echo '\n[2] VERIFICAÇÃO DE CLIENTES\n'

-- 2.1 Clientes criados recentemente
\echo '2.1 - Últimos 5 clientes cadastrados:'
SELECT 
  id,
  nome,
  telefone,
...
\echo '\n2.3 - Telefones NÃO normalizados (formato esperado: 55DDNNNNNNNNN):'
SELECT 
  id,
  nome,
  telefone,
  ...
  CASE 
    WHEN telefone ~ '^55[0-9]{10,11}$' THEN '✅ OK'
    ELSE '⚠️ FORMATO INCORRETO'
  END as status
FROM clientes
WHERE telefone !~ '^55[0-9]{10,11}$';

-- 2.4 Clientes com agendamentos
\echo '\n2.4 - Estatísticas de agendamentos por cliente:'
SELECT 
  c.nome,
  c.telefone,
...
-- ================================================
-- 3. ESPECIALIZAÇÃO FUNCIONÁRIO x SERVIÇOS
-- ================================================
\echo '\n[3] VERIFICAÇÃO DE ESPECIALIZAÇÕES\n'

-- 3.1 Mapa completo de especializações
\echo '3.1 - Mapa de especializações por funcionário:'
SELECT 
  u.nome as funcionario,
...
-- 3.2 Serviços sem nenhum funcionário habilitado
\echo '\n3.2 - Serviços SEM funcionário habilitado (problema potencial):'
SELECT 
  s.id,
  s.nome,
  s.categoria,
...
-- 3.3 Detalhamento de especializações por serviço
\echo '\n3.3 - Funcionários habilitados por serviço (amostra dos 10 primeiros):'
SELECT 
  s.nome as servico,
  s.categoria,
  u.nome as funcionario,
  fs.nivel_habilidade,
  CASE WHEN fs.ativo THEN '✅ Ativo' ELSE '❌ Inativo' END as status,
  fs.observacoes
FROM servicos s
...
-- ================================================
-- 4. HISTÓRICO DE CONVERSAS (N8N)
-- ================================================
\echo '\n[4] VERIFICAÇÃO DE CONVERSAS N8N\n'

-- 4.1 Últimas conversas registradas
\echo '4.1 - Últimas 5 conversas do N8N:'
SELECT 
  hc.id,
  c.nome as cliente,
  c.telefone,
...
-- 4.2 Estatísticas de conversas por cliente
\echo '\n4.2 - Estatísticas de conversas por cliente:'
SELECT 
  c.nome,
  c.telefone,
...
-- ================================================
-- 5. FUNÇÕES RPC - TESTES FUNCIONAIS
-- ================================================
\echo '\n[5] TESTE DE RPCs CRÍTICAS\n'

-- 5.1 Teste: listar_funcionarios_por_servico
\echo '5.1 - Teste RPC: listar_funcionarios_por_servico (exemplo: plas-06):'
...
DECLARE
  v_telefone TEXT;
BEGIN
  -- Pegar telefone do último cliente com agendamento
...
DECLARE
  ...
  v_pode BOOLEAN;
BEGIN
  -- Pegar primeiro funcionário e serviço vinculado
  ...
  IF v_funcionario_id IS NOT NULL THEN
    ...
    RAISE NOTICE 'Funcionário % pode executar serviço %: %', v_funcionario_id, v_servico_id, v_pode;
  END IF;
END $$;
...
-- ================================================
\echo '\n[6] VERIFICAÇÃO DE TIMESTAMPS\n'

-- 6.1 Registros com created_at futuro (erro de timezone)
\echo '6.1 - Registros com created_at no FUTURO (deveria estar vazio):'
SELECT 
...
\echo '\n6.2 - Agendamentos PASSADOS com status não concluído:'
SELECT 
  a.id,
  c.nome as cliente,
  s.nome as servico,
...
\echo '7.1 - Estatísticas gerais do sistema:'
SELECT 
...
-- ================================================
-- 8. VERIFICAÇÕES DE SEGURANÇA (RLS)
-- ================================================
\echo '\n[8] VERIFICAÇÃO DE POLÍTICAS RLS\n'

\echo '8.1 - Tabelas com RLS HABILITADO:'
SELECT 
  schemaname,
  tablename,
...
\echo '\n8.2 - Total de políticas RLS por tabela:'
SELECT 
  tablename,
...
\echo '2. Corrigir inconsistências encontradas'
\echo '3. Re-executar auditoria'
